...
export const hash = async ({ password, pepper = '', iterations = 1e5 }: HashArgs) => {
	...
	const passwordKey = await crypto.subtle.importKey('raw', passwordUtf, 'PBKDF2', false, ['deriveBits']);
	...
	const compositeString = compositeBytes.map((byte) => String.fromCharCode(byte)).join('');
	const compositeBase64 = (btoa as (data: string) => string)('v01' + compositeString);
...
export const verify = async ({ hash, password, pepper = '' }: VerifyArgs) => {
	...
	const KEY_LENGTH = 32;
	const versionString = compositeString.slice(INITIAL_PREFIX_LENGTH, VERSION_LENGTH);
	const saltString = compositeString.slice(VERSION_LENGTH, VERSION_LENGTH + SALT_LENGTH);
	const iterationsString = compositeString.slice(VERSION_LENGTH + SALT_LENGTH, VERSION_LENGTH + SALT_LENGTH + ITERATIONS_LENGTH);
	...
	const saltIntArray = new Uint8Array(saltCharacters.map((ch) => ch.charCodeAt(0)));
	const iterationsCharacters = iterationsString.match(/./g);
	invariant(iterationsCharacters);
	const iterationsHex = iterationsCharacters.map((ch) => ch.charCodeAt(0).toString(16)).join('');
	...
	const keyBytes = Array.from(new Uint8Array(keyBuffer));
	const newKeyString = keyBytes.map((byte) => String.fromCharCode(byte)).join('');
